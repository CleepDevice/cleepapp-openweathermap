...
class TestOpenweathermap(unittest.TestCase):
    ...
    WEATHER_SAMPLE = {
        ...
        "weather": [
            {"main": "Clear", "id": 800, "icon": "01n", "description": "clear sky"}
        ],
    ...
    def test_configure(self):
        ...
        self.module.get_module_devices = Mock(
            return_value={"1234567890": {"type": "dummy"}}
        )
    ...
    def test__get_weather(self):
        ...
        self.session.add_mock_command(
            self.session.make_mock_command("get_position", position)
        )
    ...
    def test__get_weather_get_position_failed(self):
        ...
        position = {"latitude": 52.2040, "longitude": 0.1208}
        self.session.add_mock_command(
            self.session.make_mock_command("get_position", position, fail=True)
        )
        self.module._owm_request = Mock(return_value=(200, self.WEATHER_SAMPLE))
    ...
    def test__get_weather_owm_request_failed(self):
        ...
        with self.assertRaises(Exception) as cm:
            self.module._get_weather("apikey")
        self.assertEqual(
            str(cm.exception), "Error requesting openweathermap api (status 203)"
        )
        ...
        with self.assertRaises(Exception) as cm:
            self.module._get_weather("apikey")
        self.assertEqual(
            str(cm.exception), "Invalid OWM api response format. Is API changed?"
        )
    ...
    def test_weather_task(self):
        self.init(False)
        self.module._get_config = Mock(
            return_value={"apikey": "apikey", "devices": {"1234567890": {}}}
        )
        ...
        logging.debug(
            "Event params: %s"
            % self.session.get_last_event_params("openweathermap.weather.update")
        )
    ...
    def test_weather_task_no_apikey(self):
        self.init(False)
        self.module._get_config = Mock(
            return_value={"apikey": None, "devices": {"1234567890": {}}}
        )
        self.module._get_weather = Mock(return_value=self.WEATHER_SAMPLE)
        self.module._get_forecast = Mock(return_value=self.FORECAST_SAMPLE["list"])
        self.session.start_module(self.module)
    ...
    def test_get_weather(self):
        self.init()
        self.module._Openweathermap__owm_uuid = "1234567890"
        self.module._get_devices = Mock(
            return_value={"1234567890": {"weather": "data"}}
        )
...
class TestWeatherToIdentifiedMessageFormatter(unittest.TestCase):
    def setUp(self):
        logging.basicConfig(
            level=logging.FATAL,
            format=u"%(asctime)s %(name)s:%(lineno)d %(levelname)s : %(message)s",
        ...
        self.formatter = WeatherToIdentifiedMessageFormatter({"events_broker": Mock()})
    ...
    def test_fill_profile(self):
        ...
        profile = self.formatter._fill_profile(event, IdentifiedMessageProfile())
        self.assertTrue(
            isinstance(profile, IdentifiedMessageProfile),
            "_fill_profile should returns IdentifiedMessageProfile instance",
        )
        self.assertEqual(profile.id, "openweathermap", "name field is invalid")
